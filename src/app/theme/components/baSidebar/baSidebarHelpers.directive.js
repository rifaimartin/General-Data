...
(function () {
  ...
  function BaSidebarTogglingItemCtrl($scope, $element, $attrs, $state, baSidebarService) {
    ...
    if (menuItem && menuItem.subMenu && menuItem.subMenu.length) {
      ...
      var subItemsStateViews = baSidebarService.getAllStateViewsRecursive(menuItem);
      ...
      $scope.$on('$stateChangeStart', function (event, toState) {
        if (!_isState(toState) && !_isStateView(toState) && $element.hasClass('ba-sidebar-item-expanded')) {
          vm.$collapse();
          $element.removeClass('ba-sidebar-item-expanded');
        }
      });
    ...
    function _isStateView(state) {
      return state && subItemsStateViews.some(function(subItemState) {
            return state.name.indexOf(subItemState) == 0;
          });
    }
  }
