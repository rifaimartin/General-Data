...
gulp.task('inject', ['scripts', 'styles', 'injectAuth', 'inject404', 'copyVendorImages'], function () {
  var injectStyles = gulp.src([
    ...
    path.join(conf.paths.tmp, '/serve/app/custom.css'),
...
gulp.task('datajson', [], function () {
  return gulp.src([path.join(conf.paths.src, '/user.json')])
    .pipe(gulp.dest(path.join(conf.paths.tmp, '/serve')));
});
...
gulp.task('injectAuth', ['stylesAuth'], function () {
  return injectAlone2({
    ...
    js: [path.join(conf.paths.src, '/app/**/*.setting.js'),path.join(conf.paths.src, '/app/**/*.login.js')],
    paths: [path.join(conf.paths.src, '/auth.html'), path.join(conf.paths.src, '/reg.html'),path.join(conf.paths.src, '/login.html'),path.join(conf.paths.src, '/forgot-password.html')]
  })
});
...
var injectAlone2 = function (options) {
  var injectStyles = gulp.src(
    options.css
    , {read: false});
